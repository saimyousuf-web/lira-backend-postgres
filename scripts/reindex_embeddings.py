"""
One-time re-index of all existing chunks into Qdrant using the local bge-large-en-v1.5
embeddings (replaces the old Titan vectors).

Chunk text already lives in Postgres (`chunks.txt`), and the RAG query path only needs
`chunk_id` + `course_id` from the vector store (the rest is hydrated from Postgres). So we
rebuild the vector store purely from Postgres, no source-file re-extraction.
...
def _recreate_collection():
    """Drop and recreate the collection (unnamed 1024-dim cosine) + course_id index."""
...
def _reindex(rows) -> int:
    ...
    for start in range(0, len(rows), BATCH_SIZE):
        ...
        vectors = embed_texts([row[4] or "" for row in batch])
...
async def main():
    print(f"Recreating Qdrant collection '{QDRANT_COLLECTION}' (unnamed 1024-dim, cosine + course_id index)...")
